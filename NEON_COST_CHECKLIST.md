...
3. **Metrics**タブを開く
4. 以下を確認：
   - [ ] **Active connections**: アイドル時は0に近いか
   - [ ] **Compute hours**のグラフ: スケールダウンが発生しているか（グラフが0に下がる時間帯があるか）
   - [ ] **Connection pool usage**: 接続プールの使用状況

**確認ポイント**:
- アイドル時でも接続が維持され続けている場合 → 問題あり
- Compute hoursが常に一定 → Scale to zeroが機能していない可能性
- Compute hoursが0に下がる時間帯がある → Scale to zeroが機能している
...
3. 現在の設定を確認：
   - [ ] 有効になっているか
   - [ ] 何分に設定されているか（デフォルトは5分）

**推奨設定**:
- 有効にする
- 3分に短縮（より早くスケールダウン）

**注意**: 短縮しすぎると、頻繁なアクセスがある場合にパフォーマンスに影響する可能性があります。まずは5分のまま確認し、必要に応じて調整してください。
