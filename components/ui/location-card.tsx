...
export default function LocationCard({ location }: LocationCardProps) {
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-[4/3] overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200">
        {location.topImageUrl ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
            <MapPin className="h-12 w-12 sm:h-14 sm:w-14 md:h-16 md:w-16 text-gray-400" />
          </div>
        )}
        ...
        {/* 上部説明文（日本語、中央上部） - iPhone 16最適化 */}
        {location.description && (
          <div className="absolute top-4 sm:top-5 md:top-6 left-0 right-0 text-center z-20">
            <p className="text-white text-xs sm:text-sm md:text-base font-bold leading-relaxed px-3 sm:px-4" style={{
        ...
        {/* メインタイトル（Slug、中央上部寄り、大きく太字） - iPhone 16最適化 */}
        <div className="absolute inset-0 flex items-start justify-center pt-8 sm:pt-10 md:pt-12 lg:pt-16 z-20">
          <h3 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-white uppercase tracking-tight text-center px-3 sm:px-4" style={{
        ...
        {location.workspaces && location.workspaces.length > 0 && (
          <div className="absolute bottom-4 sm:bottom-5 md:bottom-6 left-4 sm:left-5 md:left-6 z-20">
            <span className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-black/80 backdrop-blur-md text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-wider rounded-full border border-white/20">
      ...
      {/* テキストブロック - iPhone 16最適化 */}
      <div className="bg-white px-4 sm:px-5 md:px-6 lg:px-8 py-4 sm:py-5 md:py-6 lg:py-8 relative">
        {/* 詳細情報 */}
        <div className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3 font-light">
          {location.country}
        </div>
        
        {/* タイトル（都市名） */}
        <h4 className="text-xl sm:text-2xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-gray-900 via-emerald-700 to-gray-900 sm:group-hover:from-emerald-600 sm:group-hover:via-teal-600 sm:group-hover:to-cyan-600 transition-all duration-500">
          {location.city}
        </h4>
      </div>
