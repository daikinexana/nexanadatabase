...
export async function POST(request: NextRequest) {
  try {
    ...
    const { companyName, name, email, openCallUrl, action, comment } = body;
    ...
    // データベースに保存
    try {
      await prisma.openCallOrganizerInquiry.create({
        data: {
          companyName: companyName || null,
          name,
          email,
          openCallUrl: openCallUrl || null,
          action,
      ...
      // ログにも出力（デバッグ用）
      console.log("=== 公募主催者お問い合わせ（DB保存済み） ===");
      console.log("会社名:", companyName || "未入力");
      console.log("お名前:", name);
      console.log("メールアドレス:", email);
      console.log("対象の公募URL:", openCallUrl || "未入力");
      ...
      // データベースエラーでもログには出力
      console.log("=== 公募主催者お問い合わせ（DB保存失敗、ログのみ） ===");
      console.log("会社名:", companyName || "未入力");
      console.log("お名前:", name);
      console.log("メールアドレス:", email);
      console.log("対象の公募URL:", openCallUrl || "未入力");
