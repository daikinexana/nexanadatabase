...
export async function POST(request: NextRequest) {
  try {
    ...
    const { companyName, name, email, workspaceUrl, action, comment } = body;
    ...
    // アクションの値チェック
    const validActions = ["add", "remove", "update", "other"];
    ...
    // データベースに保存
    try {
      await prisma.workspaceOrganizerInquiry.create({
        data: {
      ...
      // ログにも出力（デバッグ用）
      console.log("=== ワークスペース運営者お問い合わせ（DB保存済み） ===");
      console.log("会社名:", companyName || "未入力");
      console.log("お名前:", name);
      console.log("メールアドレス:", email);
      console.log("対象のワークスペースURL:", workspaceUrl || "未入力");
      ...
      // データベースエラーでもログには出力
      console.log("=== ワークスペース運営者お問い合わせ（DB保存失敗、ログのみ） ===");
      console.log("会社名:", companyName || "未入力");
      console.log("お名前:", name);
      console.log("メールアドレス:", email);
      console.log("対象のワークスペースURL:", workspaceUrl || "未入力");
