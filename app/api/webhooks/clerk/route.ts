  ...
  if (eventType === "user.created") {
    ...
    try {
      ...
      if (!existingUser) {
        ...
        console.log(`User created via webhook: ${email_addresses[0]?.email_address}`);
      } else {
  ...
  if (eventType === "user.updated") {
    ...
    try {
      ...
      if (existingUser) {
        ...
        console.log(`User updated via webhook: ${email_addresses[0]?.email_address}`);
      } else {
        // ユーザーが存在しない場合は新規作成
        await prisma.user.create({
          data: {
        ...
        console.log(`User created via webhook (update event): ${email_addresses[0]?.email_address}`);
      }
    } catch (error) {
